...
if __name__ == "__main__":
    ...
    parser.add_argument('--out', type=str, default="trials.txt")
    ...
    # f = open("output.txt", "a")
    # print(f"World: {world_name}, Navigation: {status} with time {curr_time - start_time}, Navigation Metric: {nav_metric}")
    # f.close()
    
    with open(args.out, "a") as f:
        f.write("World_id: %d Success: %d Collided: %d Timeout: %d Time: %.4f Nav_Metric: %.4f\n" %(args.world_idx, success, collided, (curr_time - start_time)>=100, curr_time - start_time, nav_metric))
